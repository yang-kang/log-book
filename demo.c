...
typedef struct stack shen;
void zhi(shen *p,int n)
{
	p->size=n;
	p->data=kzalloc(sizeof(int)*n,GFP_KERNEL);
	p->i=0;
}
void shuru(shen *p,int data)
{
	if(p->i==p->size)
	{
		printk(KERN_INFO "zhan man ");
		return ;
	}
	p->data[p->i]=data;
	p->i++;
...
static long ok6410_leds_unlocked_ioctl(struct file *file,unsigned int cmd,unsigned long arg)
{
	...
	if(cmd==CMD1)
	{
		S=kmalloc(sizeof(struct stack),GFP_KERNEL);
	...
	if(cmd==CMD2)
	{
		printk(KERN_INFO "shu chu wei :%d",shuchu(S));	
	}
	if(cmd==CMD3)
	{
		printk(KERN_INFO "xie ru de shi %d",(int )arg);
		shuru(S,(int)arg);
	}
