...
void put_link(struct head_link *head)
{
...
struct head_link* paixu(EM *head,EM *pnew)
{
	struct head_link *p1;
	struct head_link *p2;
	p1=p2=head;
...
int main()
{
	...
	while(1)
	{
		...
		case 1:
			pnew=scanf_link();
			head=paixu(head,pnew);n++;break;
		case 2:
			printf("请输入要查找的节点：");
			scanf("%s",arr); 
			put_p(search_link(head,arr));break;
		case 3:
			printf("请输入要删除的节点：");
			scanf("%s",arr);
			head=delete_link(head,arr);
			put_link(head);n--;break;
		case 4:
			put_link(head);break;
		case 5:
			fp1=fopen("date","w");
